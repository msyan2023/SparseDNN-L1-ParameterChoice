...
def Training_print(NUM_dict, *args, **kwargs):
...
def Training(train_dataset, test_dataset, lambd, CheckStop, Exp, num_epoch=200):   
    ...
    history = {'TrainAccuracy': [], 'TestAccuracy': [], 'NNZ': [], 'lambd': lambd}
    
    NNZ_dict = {'conv1': 0, 'conv2': 0, 'conv3': 0, 'fc1': 0, 'fc2':0} ## dictionary storing nonzero weights for each layer 
    ...
    for epoch in range(num_epoch):
        ...
        for mini_batch_index, mini_batch_data in enumerate(trainloader, 0):
            ...
            if epoch % 10 == 0 and mini_batch_index % 200 == 0:  # print every 10 epoch and 200 mini-batches per epoch
        ...
        if epoch % 10 == 0 or epoch == num_epoch-1 or meetStopSign:
            ...
            print("------------------")
            print(f"meetStopSign: {meetStopSign}\n")
            print(f"lambd: {lambd}\n")
            print('TrainAccuracy over %d training samples: %0.6f %%' % (
                    NumTrain, TrainAccuracy*100))
            print('TestAccuracy over %d testing samples: %0.6f %%\n' % (
